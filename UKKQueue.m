...
- (unsigned int)
inodeForPathInQueue:(NSString *)path;
{
	int x = [watchedPaths indexOfObject:path];
	int fd = 0;
	
	if ( x != -1 )
	{
		NSNumber *n = [watchedFDs objectAtIndex:x];
		fd = [n intValue];
	}
	else return 0;
...
-(void)		watcherThread: (id)sender
{
    ...
    while( queueFD != -1 )
    {
		...
		NS_HANDLER
			NSLog(@"LayerLink error: Error in UKKQueue: %@",localException);
		NS_ENDHANDLER
...
{
	if( delegate )
		[delegate kqueue: self receivedNotification: nm forFile: fp];
	else
