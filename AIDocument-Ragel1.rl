...
# end structs
	...
	# the layerStart variable always holds the location of the last '/' in the stream
	layerStart = ( any | ('/' >0 >layerStart) )* $1 $incrementLoc;
	...
	init {
		fsm->layerName = [[PDFName nameWithString:@"Layer"] retain];
	}
	...
	action name {
		PDFName *tmpName = [PDFName nameWithString: [NSString stringWithCString:(char *)[fsm->nameChars bytes]]];
		
		if ( [tmpName isEqual:fsm->layerName] )
		{
			[fsm->array addObject:[NSNumber numberWithInt:fsm->layerStart]];
		}
		
		[fsm->nameChars appendBytes:&fsm->nullByte length:1];
		[fsm->self addToken:tmpName];
	...
	main := PDFStreamParsingMachine_main | layerStart;
%%
...
- (NSArray *)locateLayerStarts;
{
	...
	void *buf = (void *)[streamData bytes];
	int buffer_size = [streamData length];
	struct machineAll fsm;
	
	machineAll_init( &fsm );
	fsm.array = array;
	fsm.self = nil; // unused
	machineAll_execute( &fsm, buf, buffer_size );
	
	[fsm.layerName release];
...
# end structs
	...
	action number {
		...
		{
			[fsm->self addToken:
								[PDFRealNumber realNumberWithString:
														[NSString stringWithCString:(char *)[fsm->numChars bytes]]]];
		}
		else
		{
			[fsm->self addToken:
								[PDFIntegerNumber integerNumberWithString:
														[NSString stringWithCString:(char *)[fsm->numChars bytes]]]];
		}
	...
	struct {
		...
		PDFName *aiName;
		int currentLayer;
		BOOL currentLayerIsHidden;
		BOOL captureHiddenLayerName;
	};
	
	init {
		fsm->captureHiddenLayerName = NO;
		fsm->currentLayerIsHidden = YES; // this is the correct initialization value
		fsm->layerName = [[PDFName nameWithString:@"Layer"] retain];
		fsm->aiName = [[PDFName nameWithString:@"AltAI8"] retain];
	}
	...
	action name {
		...
		if ( [tmpName isEqual:fsm->layerName] )
		{
			if ( ! fsm->currentLayerIsHidden )
			{
				[fsm->array addObject:[NSNumber numberWithBool:NO]];
			}
			fsm->currentLayerIsHidden = NO;
		}
		else if ( [tmpName isEqual:fsm->aiName] )
		{
			fsm->currentLayerIsHidden = YES;
			fsm->captureHiddenLayerName = YES;
		}
		else
		{
			if ( fsm->captureHiddenLayerName )
			{
				fsm->captureHiddenLayerName = NO;
				[fsm->array addObject:tmpName];
			}
		}
	...
	main := PDFStreamParsingMachine_main;
%%
...
- (NSArray *)locateHiddenLayers;
{
	...
	void *buf = (void *)[streamData bytes];
	int buffer_size = [streamData length];
	struct machineHidden fsm;
	
	machineHidden_init( &fsm );
	fsm.array = array;
	fsm.self = nil; // unused
	machineHidden_execute( &fsm, buf, buffer_size );
