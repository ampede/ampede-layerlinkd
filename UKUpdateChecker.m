...
-(void) awakeFromNib
{
	NSNumber	*   doCheck = [[NSUserDefaults standardUserDefaults] objectForKey: @"checkForSoftwareUpdatesAtStartup"];
	NSString	*   appName = @"LayerLink"; // [[NSFileManager defaultManager] displayNameAtPath: [[NSBundle mainBundle] bundlePath]]; 
	NSNumber	*   lastCheckDateNum = [[NSUserDefaults standardUserDefaults] objectForKey: @"lastCheckDate"];
	...
	if( doCheck == nil )		// No setting in prefs yet? First launch! Ask!
	{
		...
		[[NSUserDefaults standardUserDefaults] setObject: doCheck forKey: @"checkForSoftwareUpdatesAtStartup"];
	}
	...
	if( [doCheck boolValue] )
	{
		...
		if( timeSinceLastCheck > (3600 *24 *DAYS_BETWEEN_CHECKS) )
		{
			...
			[[NSUserDefaults standardUserDefaults] setObject: [NSNumber numberWithDouble: [NSDate timeIntervalSinceReferenceDate]] forKey: @"lastCheckDate"];
		}
	}
}
    ...
    if( appVersionDict != nil )		// We were able to download a dictionary?
	{
		...
		else if( newVersion != nil )	// If there's an entry for this app:
		{
            ...
            int button = NSRunAlertPanel(
							...
							@"Download", NSLocalizedStringFromTable(@"Cancel", @"UKUpdateChecker", @""), nil,
							newVersion );
...
-(void)			setCheckAtStartup: (BOOL)shouldCheck
{
	...
	[[NSUserDefaults standardUserDefaults] setObject: [NSNumber numberWithDouble: 0] forKey: @"lastCheckDate"];
}
